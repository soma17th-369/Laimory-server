...
수 있으므로 oldest age가 30시간을 초과한 상태가 15분 지속될 때 경고한다. S3 실패·crash·process 전체
run budget 초과분은 다음 날 실행으로 이월된다. worker가 꺼져 있어도 두 gauge는 MySQL job table에서 계속
관측된다. dev WAS에서는 전체 환경을 출력하지 말고 각 container의
...
flag가 true인데 backlog가 줄지 않으면 직전 03:00 KST에 두 app process가 가용했는지, claimed/deferred/
completed 증가량과 process-wide run budget, MySQL/Hikari 상태, S3/IAM 오류, delete attempt 실패와 batch
duration을 차례로 확인한다. 실패
