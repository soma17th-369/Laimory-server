...
- 엣지케이스(경합·ms 단위 타이밍·장애 중첩 등 드문 조건) 대응은 기본으로 구현하지 않는다. 필요하다고
  판단되면 발생 트리거·발생 빈도·얻는 것·잃는 것을 정리해 사용자 승인을 받은 뒤에만 넣고, 승인 전에는
  단순한 경로를 유지한다. 정상 입력 검증·만료 token·외부 API 실패처럼 상시 발생하는 실패 처리는
  엣지케이스가 아니다.
