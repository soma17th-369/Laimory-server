...
- 드문 조건(경합·ms 단위 타이밍·장애 중첩 등) 대응이 코드를 복잡하게 만들면(경계 분리·래퍼·우회 경로·
  전용 테스트 등) 기본으로 구현하지 않는다. 필요하다고 판단되면 발생 트리거·발생 빈도·얻는 것·잃는 것을
  정리해 사용자 승인을 받은 뒤에만 넣고, 승인 전에는 단순한 경로를 유지한다. 기존 경로 안에서 단순하게
  처리되는 드문 조건과 상시 발생하는 실패 처리(정상 입력 검증·만료 token·외부 API 실패)는 승인 없이 다룬다.
...
- 계획 문서에는 "엣지케이스 판단" 절을 두고 항목마다 발생 트리거·발생 빈도·얻는 것·잃는 것·
  결정(승인됨/승인 필요)을 적는다.
