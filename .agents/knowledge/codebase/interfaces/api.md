...
| `/t/api/{version}` | dev 전용 테스트 API, endpoint별 자체 인증 | 활성화 property가 없으면 controller 빈 자체가 없어 mapping 부재(404). 켠 환경에서는 endpoint가 전용 opaque Bearer token을 강제 |
...
token 누락·불일치는 401 `-2001`, 비활성 환경은 경로 부재로 404다 — **새 error code는 추가하지 않았다.**
노출은 `app.ai.timeline-test.enabled` 하나가 소유하며 기본이 off라 미설정·prod에서는 OpenAPI 문서에도
라우팅에도 존재하지 않는다. 인증은 회원 access JWT가 아닌 설정 주입 opaque Bearer token 하나와의 상수
시간 대조이고 draft의 회전 task token 계약과 무관하다. 계약 상세는 [ai-contract](ai-contract.md)가 소유한다.
