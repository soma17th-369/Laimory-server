...
`/enabled`는 **모든 push의 전체 스위치**다(#367) — 예정 알림뿐 아니라 타임라인 완료 통지도 OFF면
발송하지 않는다. 마스터 OFF가 곧 전체 차단이라야 탈퇴가 FID를 지우지 않고도 발송을 막을 수 있기
때문이다. 알림별 설정값은 보존하므로 다시 켜면 이전 설정으로 재개한다. 종류가 하나뿐인 현재는
마스터 OFF와 리마인더 OFF의 효과가 같다.
