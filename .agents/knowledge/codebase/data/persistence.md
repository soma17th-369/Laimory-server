...
`term_documents`(#303/#432)는 버전별 불변 약관 문서다. 핵심 컬럼은 `term_type`·`version`·`title`·
`content_url` 네 개이고 `(term_type, version)`이 복합 PK다. 원문은 담지 않고(#320) 게시된 버전별 page가
소유하며 이 행은 게시 시점에 확정한 주소만 저장한다. Server는 원문을 서빙·proxy하지 않고
`content_url`을 slug/version으로 역산하지 않는다. URL이 실제로 200인지는 요청·기동 중 확인하지 않고
게시 게이트가 검증한다.
...
`TimelineEventPhotoAddService.link`만 연결 transaction에서 표시를 해제한다. 공통 감사 동작은 유지한다.
처리 종료 뒤 담당 전체의 72시간 이상 관측된 고아를 별도 transaction에서 집계해 ERROR로 알린다.
job으로 넘긴 Item은 제외하며 미관측 Item의 과거 고아 전환 시각을 추정하지 않는다.

#474의 V2는 초안 선점 컬럼·인덱스를 제거한다. V1과 동결 fixture는 그대로 유지한다.
구버전 호환을 위한 Flyway target=1 코드 전환 → 구버전 제거 → V2 적용 및 복구 순서는
[정리 스케줄러 전환](../../../../docs/database/474-scheduler-rollout.md)을 따른다.
