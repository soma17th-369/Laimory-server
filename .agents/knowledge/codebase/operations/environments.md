...
- `deploy/README.md`, `deploy/was/README.md`, `deploy/monitoring/*`
...
| prod | default | live configuration outside repository | unverified | unverified | unverified | property default off | JSON intended | unverified | no app deploy workflow |
...
추가한다(절차는 `deploy/was/README.md`). `APP_COMMIT_SHA`는 배포 workflow가 `.env`에
원자 upsert하는 유일한 key다.

prod 행은 repository에서 확인 가능한 범위만 기록한다. production workflow와 complete runtime
configuration source가 없으므로 live 값은 별도 조회 없이는 확정하지 않는다.
...
dev monitoring은 별도 private host에 있고 prod는 수집하지 않는다. monitoring
...
rg -n 'APP_ENV|APP_AI_MODE|APP_GEO_MODE|SWAGGER_ENABLED|REDIS_KEY_PREFIX' \
  src/main/resources .github/workflows deploy
```
