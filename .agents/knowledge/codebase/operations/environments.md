...
- dev 전용 AI 동기 테스트 endpoint(#394 — `app.ai.timeline-test.*`, **`APP_AI_MODE`와 완전히 별개
  ...
  `APP_AI_TIMELINE_TEST_TOKEN`(호출자 인증용 opaque token — 앱은 SHA-256 digest만 보관),
  `APP_AI_TIMELINE_TEST_AI_AUTH_TOKEN`(선택 — AI가 자체 인증을 요구할 때만),
  `APP_AI_TIMELINE_TEST_CONNECT_TIMEOUT`, `APP_AI_TIMELINE_TEST_READ_TIMEOUT`(AI
  `PIPELINE_TIMEOUT_SEC`(120s)보다 길어야 하며 위반은 기동 실패),
  `APP_AI_TIMELINE_TEST_MAX_REQUEST_BYTES`, `APP_AI_TIMELINE_TEST_MAX_RESPONSE_BYTES`.
  활성화하면 URL·token 누락·형식 오류가 전부 기동 실패다(fail-fast). deploy pre-flight 검증 대상은
  아직 아니다 — prod 차단은 앱 기본값 off 하나에 의존한다.
