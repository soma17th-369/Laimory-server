...
1. `JwtAuthenticationFilter`(security chain 내부, `AuthorizationFilter` 앞)가 Bearer token을
   `JwtTokens.parseUserId`로 검증하고, 성공 시 `UserAccountService#isActive(userId)`까지 통과한
   경우에만 `Long` userId principal 인증을 SecurityContext에 넣고 userId 로그 attribute를 심는다(#305).
   이 검사는 #429부터 공유 Redis 캐시를 탄다(#441부터 `UserAccountService.isActive`에 직접 부착) —
   hit는 공유 Redis만 보고, miss는 users PK existence 조회 뒤 ACTIVE=true만 적재한다
   (음성 미캐시; 규칙·정책은 아래 "탈퇴 차단 정책(#429)").
...
ACTIVE 검사의 캐시 규칙: **ACTIVE=true만** 캐시(음성 미적재 — `@Cacheable`의 `unless`),
무효화는 탈퇴 orchestrator의 commit 후 **evict 하나**(갱신 경로 없음), TTL 15분은 evict 유실 대비
안전망으로 **쓰기 시점 고정**(조회가 연장하지 않음)이다. #441부터 발급·회전(`AuthTokenService`)도
필터와 같은 캐시를 경유한다 — 검사 위치는 `UserAccountService.isActive` 하나이고 wrapper·경계
arch test는 없다.

탈퇴 시점 기준의 유한한 hard bound는 선언하지 않는다(fencing·발급 직렬화·완료시간 상한 비채택의
귀결). 대신 두 가지를 보장한다: ① 탈퇴 커밋·evict 뒤 **시작된** 필터 검사·발급·회전은 miss→DB
도달로 결정적으로 거절된다 — evict 유실·늦은 적재로 stale 엔트리가 남은 창(엔트리 수명 ≤TTL)
에서만 인증·발급·회전이 한시적으로 통과할 수 있고(#441 — 창 종료 후에는 그 산물도 필터·회전
재검사가 DB로 거절한다), 그 외 노출은 커밋 전에 검사를 통과한 in-flight 작업의 산물뿐이다.
② 각 access token은 실제 발급 시각 + `app.auth.jwt.access-ttl`(기본 15m) + 만료 leeway 60s까지만
유효하고, stale 캐시 엔트리도 각각 적재 시각 + TTL에 소멸한다. TTL은 지배적
실용 상한(기대치)이다. 응답 계약(401 `-2001` 수렴·존재 비노출)은 변하지 않았다.

약관 동의는 인증 API에서 강제하지 않는다(#436 — #303의 enforcement gate 제거). 동의 보장은 가입
flow의 클라이언트 책임이고(가입이 되어 있으면 동의는 전제된 상태), 약관 개정 후 동의 필요 여부는
앱 초기화 API 응답 `terms.agreementRequired`(#434)가 알려준다. 인증을 통과한 `/a/api` 요청은 약관
동의 여부와 무관하게 controller에 도달하며 403 `-3001`은 더 이상 반환되지 않는다.
...
5. access token은 HS256이며 기본 15분, `iss/sub/iat/exp` claim만 둔다. 서버에 저장하지 않는다.
   ...
   refresh 재사용 탐지만 유지한다. 이 발급 전 검사는 #441부터 필터와 같은 공유 Redis 캐시를
   경유한다(위 "탈퇴 차단 정책"). 검사 통과 직후 탈퇴와 겹친 in-flight 발급은 허용된 제한 예외이며
   그 token도 `/a/api` ACTIVE 검사와 다음 회전 검사(둘 다 탈퇴 evict 뒤 miss부터)에서 거절된다.
...
- 회원 account API(`GET/DELETE /a/api/{version}/user`)는 principal userId를 직접 받는다. GET은
  ...
  보존되고, old credential의 사용·연장 차단은 요청·발급 전 `ACTIVE` 검사가 담당한다 — 필터·발급·
  회전 모두 commit 후 캐시 evict부터(#429 "탈퇴 차단 정책"·#441; 전량 REVOKED는 차단의 필수
...
- `/a/api` 인증과 token/refresh 발급·회전은 회원 `ACTIVE`를 확인하되 #429부터 공유 Redis 캐시를
  탄다(#441부터 두 경로 모두 — ACTIVE=true만·탈퇴 시 DEL·TTL 안전망, 한시적 stale의 범위는 "탈퇴
  차단 정책"이 소유). 회원 없음과 탈퇴는 응답으로 구분되지 않으며,
...
- SecurityContext principal은 별도 래퍼 없는 `Long` userId다. 보호 operation의 principal parameter는
  원칙적으로 하나다 — 콘텐츠·push·온보딩 완료 controller는 `@CurrentSubject UUID subjectId`를,
  회원 account controller는 hidden `@AuthenticationPrincipal Long userId`를 받는다. 유일한 예외인
  앱 초기화 GET은 온보딩(subject 소유)과 약관 동의 필요 판정(계정 raw userId 소유)을 함께 반환하므로
  두 hidden principal을 받되, 온보딩 조회에는 subjectId만, 동의 판정에는 userId만 쓴다(#434 — draft
  생성의 두 principal 예외는 #436에서 소멸). String principal을 만드는 테스트 헬퍼 `user()`는 쓰지
  않고 `AuthTestSupport`를 쓴다.
