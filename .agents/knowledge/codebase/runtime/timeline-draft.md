...
- request rawId는 입력 순서의 첫 항목을 사용한다. 대상 Event에 같은 rawId가 이미 연결돼 있으면 no-op, 아니면
  새 Item이다(#502) — record의 다른 Event는 조회하지 않고 저장본과 요청을 비교하지 않는다. no-op에 도달하는
  것은 커밋 뒤 응답을 잃은 같은 PATCH의 재시도뿐이다(Android는 사진 선택마다 새 rawId·filename을 발급).
...
- 삭제된 PHOTO 재추가는 새 upload identity다. Android는 같은 로컬 사진을 다시 선택해도 새 presign
  ...
  대상 Event에 있으면 no-op, 아니면(첫 요청이 롤백됐으면) 새 Item으로 저장된다.
