...
2. **결과 저장**(`POST .../result`, 입력 응답의 `Task-Token`): shape 검증 뒤·선점 전에
   ...
   단계는 치환본만 본다 — 치환 실패는 선점 없이 `RESULT_PENDING` 유지로 끝난다(원문 fallback 없음).
   치환본 payload 지문을 retry receipt에 심는 CAS로 선점한 요청만 DB 검증·시각 정규화·+10분 nudge/clamp·
   Event/Item/junction INSERT·채택 source DELETE를 하나의 MySQL transaction으로 commit한다. **선점은
   token을 바꾸지 않는다** — callback token 회전과 receipt의 `committedAt` 기록은 commit 뒤 한 번의 CAS로
   함께 일어나고, 그 뒤에야 callback token 원문을 응답한다. 저장 예외면 선점 receipt를 지운다(token이
   그대로라 AI 재시도가 정상 경로로 다시 돈다). 응답 유실 뒤 같은 result token으로 같은 결과가 다시 오면
   MySQL을 건드리지 않고 새 callback token만 재발급해 `status=ALREADY_PROCESSED`로 응답하며, 지문이
   다르면 409 `-1017`이다.
...
선점 뒤 commit 전 장애는 `committedAt`이 없어 복구되지 않고 PROCESSING TTL로 만료된다. 창을 넘긴
...
- **어느 날을 실을지는 `record_date` 오름차순으로 고른다.** 큐 순서는 진입 시각이라 기록 날짜와 다를 수
...
- retry receipt의 `committedAt`은 MySQL commit 뒤에만 쓴다 — 이 값의 존재가 "graph가 확정됐다"의 유일한
  증거이며, 없으면 어떤 재요청도 멱등 성공으로 처리하지 않는다.
