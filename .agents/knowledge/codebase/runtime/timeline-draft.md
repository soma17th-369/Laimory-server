...
- 고아 스위퍼는 후보 PK의 최초 관측 표시·시각을 먼저 commit하고 별도 transaction에서 새로 읽어
  처리한다. 처리 rollback에도 최초 기록은 유지된다. 재연결은 표시를 해제하며, 처리 종료 뒤
  담당 전체의 72시간 이상 관측된 고아를 ERROR로 알린다. 미관측 대기시간은 측정하지 않는다.
