...
- **배치는 동기다.** 사용자를 하나씩 처리하며 `result-wait`(기본 3분)까지 task key 소멸을
  `poll-interval`(기본 2초) 간격으로 확인하고, **문서가 실제로 바뀐 것을 보고 나서야** 큐에서 지운다 —
  접수 202만 보고 지우면 AI가 FAILED를 줘도 그 날이 영구히 사라져 재시도 큐가 아니게 된다. 성패는 base
  지문과의 비교로 가른다(FAILED와 409 폐기는 둘 다 DB를 건드리지 않는다). 4xx 거절·재료 없음은 다시
  보내도 같으므로 기다리지 않고 큐에서 걷어낸다. 실행 시간이 사용자 수에 비례하므로 `run-budget`
  (기본 30분)을 넘기면 나머지는 큐에 둔 채 끝낸다.
- **즉시 접수는 반대로 결과를 기다리지 않는다**(fire-and-forget). 그래서 그 경로로 접수한 뒤 AI가
  FAILED를 주면 그 날은 누락된다 — 의도적 비대칭이다(저장 응답은 이미 나갔고 기다리면 async 스레드를
  최대 3분 붙잡는다).
...
- emotion 설정 API가 없다.
- User Memory 갱신이 **끝내 안 된 날**(즉시 접수에서 AI FAILED, 또는 배치가 7일 deadline까지 못 반영)은
  그 날의 내용이 memory에 영영
