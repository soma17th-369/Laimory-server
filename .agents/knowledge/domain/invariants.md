...
- 세 정리 스케줄러는 `MOD(id - 1, serverCount * workerCount)`로 담당을 나누고 slot당 후보 최대
  batch-size(기본 250) 한 배치만 처리한다. workerIndex는 workerId * workerCount + localIndex다.
  선점용 잠금 읽기·내부 반복·고아 cursor·초안 cleanup_available_at은 없다. DML 잠금은 남는다.
...
- 관측된 Item의 updated_at은 재조회·실패 시 유지한다. 기존 Item 재연결 transaction이 표시를
  해제하며 다시 고아가 되면 새 최초 시각을 쓴다. BaseEntity·공통 AuditorAware는 변경하지 않는다.
