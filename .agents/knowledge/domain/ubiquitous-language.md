...
| 한글명 | 영문명 | 상태 | 설명 |
|---|---|---|---|
| AI 접수 요청 | AI Dispatch Request | 현재 구현 | 기존 `POST /v1/timeline` body(`taskId`·`callbackToken`·`dailyRecordId`·offset `window`)를 유지한다. source item은 싣지 않고 AI가 토큰으로 입력 조회 API를 호출한다. 필드명은 AI 규격이 명명 권위인 contract fixture다. |
