...
| 한글명 | 영문명 | 상태 | 설명 |
|---|---|---|---|
| 공지 | Notice | 현재 구현 | 관리자가 등록하는 공지 한 건(`notices`, PK `notice_id`)이다. 제목과 본문 텍스트를 행이 직접 소유한다 — 약관과 달리 외부 page URL을 가리키지 않는다. 앱은 공개 목록·상세 조회만 하고 등록·수정은 localhost 관리자 웹이 한다(#517). 수정은 제목·본문 전체 교체다. |
