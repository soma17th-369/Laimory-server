...
| 약관 종류 | Term Type | 현재 구현 | `TermType` enum 6종 — `TERMS_OF_SERVICE`(이용약관)·`SENSITIVE_INFORMATION_CONSENT`(민감정보)·`THIRD_PARTY_PROVISION_CONSENT`(제3자 제공)·`CROSS_BORDER_TRANSFER_CONSENT`(국외 이전)·`LOCATION_BASED_SERVICE_TERMS`(위치약관)·`PRIVACY_POLICY`(개인정보 처리방침)다. 6종 모두 같은 catalog와 반복 `termTypes` 공개 조회를 쓰며 enum에는 stage·순서·필수/고지 분류 속성이 없다. 공개 응답 순서는 클라이언트 요청 순서이고, 초기 동의 enforcement 대상 5종은 `TermCatalogReadiness`에 명시하며 개인정보 처리방침은 상시 공개만 한다. 게시 URL은 정책이 아니라 게시 사실이라 문서 행이 소유한다. |
...
| 필수 약관 gate | Terms Enforcement | 현재 구현 | `/a/api` HandlerMethod interceptor가 controller 진입 전에 이용약관·민감정보·제3자 제공·국외 이전·위치약관의 현재 문서 동의를 한 번에 검사한다(미동의 403 `-3001`). `@LoginTermsExempt`(동의 등록/이력·내 회원 조회·회원 탈퇴 DELETE /user·push 등록 PUT/DELETE·push 수신 설정 3종·앱 초기화 GET /initializer·온보딩 완료 POST /onboarding/complete)만 면제한다. 개인정보 처리방침은 상시 공개 문서라 gate에 포함하지 않는다. catalog가 미준비면 다섯 종류를 부분 강제하지 않고 gate 전체를 fail-open한다. |
| catalog 준비 상태 | Term Catalog Readiness | 현재 구현 | 전체 6종 seed 존재·`term_type` literal 유효성·초기 동의 대상 5종의 현재 문서 커버리지 판정(`TermCatalogReadiness`)이다. 기동 검사와 요청별 판정이 bounded 전이 로그·단일 metric(`laimory.terms.catalog.ready`·`laimory.terms.gate.fail_open`)으로 알리되 기동·공개 조회를 막지 않는다. 로그 수위는 테이블이 완전히 빈 pre-activation이면 WARN, seed 행이 존재하는 문제·ready 퇴행이면 ERROR다. |
