...
| 인증 사용자 API | Authenticated API | 현재 구현 | `/a/api/{version}`은 bearer 인증 강제 영역이다. 무토큰/무효 토큰과 비활성 회원(없음/`WITHDRAWAL_PENDING` — 매 요청 users PK ACTIVE 검사, #305)은 401 `-2001`이다. JWT filter의 raw `Long userId` principal은 유지하되, 콘텐츠·push API는 `@CurrentSubject` MVC resolver가 Java `UUID subjectId`로 해석해 controller/service에 주입한다. |
...
| 약관 종류 | Term Type | 현재 구현 | `TermType` enum 6종 — `TERMS_OF_SERVICE`(이용약관)·`PRIVACY_POLICY`(개인정보 처리방침)은 `LOGIN`, `SENSITIVE_INFORMATION_CONSENT`(민감정보)·`THIRD_PARTY_PROVISION_CONSENT`(제3자 제공)·`CROSS_BORDER_TRANSFER_CONSENT`(국외 이전)·`LOCATION_BASED_SERVICE_TERMS`(위치약관)는 `TIMELINE_FIRST_CREATE` 단계다. 6종 모두 같은 catalog와 `termTypes` 공개 조회를 쓰며 enum에 필수/고지 분류 속성은 없다. 이용약관·민감정보·제3자 제공·국외 이전의 stage gate와 위치약관 조건부 gate는 실제 enforcement 지점에 명시하고, 개인정보 처리방침은 상시 공개만 한다. enum은 `(stage, displayOrder)`를 소유하고 DB는 이 값을 복제하지 않는다. 게시 URL은 정책이 아니라 게시 사실이라 문서 행이 소유한다. |
| 약관 단계 | Term Stage | 현재 구현 | `TermStage` enum — 동의·enforcement의 공통 축(`LOGIN`, `TIMELINE_FIRST_CREATE`)이다. stage 소속 판정은 `TermType` mapping만 쓰며 DB에 stage 컬럼이 없다. 공개 조회는 stage가 아니라 반복 `termTypes` query로 필요한 종류를 직접 요청한다. |
