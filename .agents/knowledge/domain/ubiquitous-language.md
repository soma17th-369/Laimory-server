...
| 예정 알림 마스터 | Push Preference | 현재 구현 | subject별 예정 알림 전체 스위치(`push_preferences`, 기본 ON)다. OFF는 예정 알림 발송만 막고 종류별 설정값은 보존한다. 타임라인 완료 통지는 사용자가 시작한 작업의 결과라 이 스위치를 따르지 않는다. |
