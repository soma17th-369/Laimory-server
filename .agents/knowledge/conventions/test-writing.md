...
- 관측 가능한 결과를 검증한다: 응답 본문·에러 코드, DB에 남는 상태, enqueue된 작업.
  ...
  지켜질 수 있는 전제다. 내부 구조에 결합해 그렇지 못한 테스트는 허용 표의 리팩터링 경우로
  수정한다.
...
- 실제 외부 네트워크 금지. 외부 HTTP(Kakao 등)는 loopback `MockWebServer`로 production 배선
  ...
  사용자 승인이 있을 때만 예외다.
- 공유 로컬 DB 격리: 각 테스트가 자기 데이터를 스스로 만들고(예: `AtomicLong` seed로 고유 ID
  대역 확보), 실행 순서·다른 테스트가 남긴 상태에 의존하지 않는다. 모든 테스트는 단독 실행으로도
  통과해야 한다.
...
- 실패한 테스트는 원인 분류(구현 결함 / 계약 변경 / 테스트 결함 / 플레이키 / 구조 결합
  리팩터링) 없이 넘어가지 않는다.
