...
테스트 수정·삭제·비활성화가 허용되는 경우는 셋뿐이며 각각 입증 요건이 있다.
...
- `Thread.sleep` 대기 금지. 비동기 결과는 Awaitility로 조건을 폴링한다.
...
- `ApiAuthenticationContractTest` — 모든 `*Api` 인터페이스의 인증·응답 계약 선언
- `RedisAccessArchTest` — Redis 접근 경계
- `SubjectMappingAccessArchTest` — subject 매핑 접근 경계

## Invariants

- 구현 변경과 테스트 수정·삭제·비활성화가 같은 PR에 있으면, 수정된 테스트마다 허용 3경우 중
...
판정 절차(허용 3경우), 레이어 선택 기준, 결정성·격리·네이밍 규칙이 바뀔 때 갱신한다.
