...
- **서브에이전트는 사용자에게 직접 묻지 않는다.** 모호한 판단 요소가 있으면 임의로 확정하지 말고,
  물어야 할 항목·선택지·본문 초안을 정리해 메인 에이전트에 반환한다.
- 메인 에이전트가 `AskUserQuestion`으로 사용자에게 묻고("묻는 방식" 절의 제약을 그대로 적용), 8단계
  요약 확인까지 받은 뒤 **`SendMessage`로 같은 서브에이전트에 답을 전달**해 생성을 마치게 한다.
  새 `Agent` 호출은 앞서 쌓은 맥락이 날아가므로 쓰지 않는다.
- 서브에이전트는 마지막에 생성된 이슈 번호·URL·실제로 설정된 필드 값을 반환하고, 메인은 그것만 보고한다.

서브에이전트를 띄울 수 없는 환경이면 그 사실을 사용자에게 밝히고 메인 세션에서 아래 절차를 그대로 수행한다.
...
으로 선택지를 띄워 사람이 고르게 한다**(서브에이전트로 실행 중이면 메인 에이전트를 거쳐 묻는다 —
"실행 방식" 절). 절대 애매한 채로 진행하지 않는다. 확실한 것만 자동으로
...
질문은 항상 **메인 에이전트**가 던진다(서브에이전트는 물어야 할 항목만 반환한다 — "실행 방식" 절).
