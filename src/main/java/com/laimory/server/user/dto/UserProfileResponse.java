...
/**
 * 내 회원 정보 응답({@code GET /a/api/{version}/user}).
 *
 * <p>{@code userId}는 회원 행 PK(= access token의 {@code sub})라 항상 존재한다 — 가명화 subjectId가 아니라
 * 회원 account 식별자다.
 *
...
public record UserProfileResponse(
        @Schema(description = "회원 식별자 — 회원 행 PK(access token의 sub와 같은 값), 항상 존재",
