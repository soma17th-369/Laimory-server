...
/**
 * user_memory_documents leaf 서비스. 자신과 1:1인 UserMemoryRepository에만 접근한다.
 *
...
public class UserMemoryService {
    ...
    public Optional<JsonNode> find(SubjectId subjectId) {
        return userMemoryRepository.findById(subjectId.bytes()).map(UserMemory::getMemory);
    }
     ...
     */
    public void replace(SubjectId subjectId, JsonNode memory) {
        if (memory == null || memory.isNull()) {
            userMemoryRepository.deleteBySubjectId(subjectId.bytes());
            return;
        }
        userMemoryRepository.upsert(subjectId.bytes(), memory.toString(), LocalDateTime.now(clock));
    }
}
