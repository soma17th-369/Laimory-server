 ...
 *
 * <p>{@code users} insert로 userId(IDENTITY)를 확보한 <b>같은 transaction</b>에서 subject mapping을
 * insert한다 — mapping insert·secret/HMAC 처리가 실패하면 user insert까지 함께 rollback되어 부분
...
public class NewUserProvisioner {
    ...
    /**
     * user와 subject mapping을 한 transaction에서 저장한다. {@code saveAndFlush}로 insert를 즉시
     * flush해 UNIQUE 위반이 이 메서드 안에서 드러나게 한다(commit 시점으로 미루지 않음).
     */
    @Transactional
    public User provision(Provider provider, String providerUserId, String email, String nickname) {
        ...
        subjectMappingService.createFor(user.getUserId());
        return user;
    }
}
