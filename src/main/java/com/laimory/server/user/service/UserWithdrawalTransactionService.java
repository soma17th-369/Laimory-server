 ...
 * ④ subject push 등록 전부 삭제 ⑤ 종류별 알림 설정 → 푸시 마스터 → 수신 동의 snapshot 삭제
 * (FK RESTRICT라 이 순서여야 하며, 동의 증적 event는 보존한다) ⑥ userId-only PENDING 삭제 작업
 * insert-if-absent.
 *
...
public class UserWithdrawalTransactionService {
    ...
    public void withdraw(long userId) {
        ...
        // 종류별 설정 → 마스터 순서를 지킨다(FK RESTRICT). 동의 snapshot은 지우고 증적 event는 남긴다.
        scheduledNotificationPreferenceService.deleteAllForSubject(subjectId);
        pushPreferenceService.deleteForSubject(subjectId);
        notificationConsentService.deleteStateForSubject(subjectId);
