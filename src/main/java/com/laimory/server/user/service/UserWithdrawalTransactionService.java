 ...
 * 막으므로(필터·발급·회전 모두 commit 후 캐시 evict부터 — #429·#441) refresh 전량 {@code REVOKED}는
 ...
 * in-flight token 발급이 race로 늦게 저장한 ACTIVE refresh 행은 다음 회전의 ACTIVE 검사(탈퇴
 * evict 뒤 miss부터 — #429·#441)에서 거절되며 #302 정리 대상이다. 202는 물리적 zero가 아니라
 * "commit 후 시작분 차단 + in-flight 산물의 한시적 잔존(각 token은 발급 시각+수명까지)"을 뜻한다 —
