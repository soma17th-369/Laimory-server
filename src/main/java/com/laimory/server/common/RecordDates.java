...
/**
 * record_date를 클라가 보낸 벽시계 시각(recordAt)에서 서버가 권위 있게 계산한다(정오 경계).
 *
...
public final class RecordDates {
    ...
    /** 벽시계 시각(recordAt)에 정오 경계를 적용해 record_date를 도출한다(zone 불필요). */
    public static LocalDate resolveRecordDate(LocalDateTime recordAt) {
        if (recordAt == null) {
            throw new IllegalArgumentException("recordAt must not be null");
        }
        // 정오 경계: 정오 이전이면 전날, 정오(12:00) 이상이면 당일.
        if (recordAt.toLocalTime().isBefore(LocalTime.NOON)) {
            return recordAt.toLocalDate().minusDays(1);
        }
        return recordAt.toLocalDate();
    }
