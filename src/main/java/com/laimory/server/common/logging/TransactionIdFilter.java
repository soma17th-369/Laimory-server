 ...
 *       헤더는 chain 진입 전에 설정하므로 필터 단계에서 직접 쓰는 에러 응답에도 실린다.</li>
...
public class TransactionIdFilter extends OncePerRequestFilter {
    ...
    private final AccessLogBodyMasker bodyMasker;

    public TransactionIdFilter(ObjectMapper objectMapper) {
        this.bodyMasker = new AccessLogBodyMasker(objectMapper);
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        BoundedRequestCaptureWrapper requestWrapper = new BoundedRequestCaptureWrapper(
                request, AccessLogBodyMasker.CAPTURE_LIMIT_BYTES);
        BoundedResponseCaptureWrapper responseWrapper = new BoundedResponseCaptureWrapper(
                response, AccessLogBodyMasker.CAPTURE_LIMIT_BYTES);
        ...
        try {
            ...
            safeLogCompletion(requestWrapper, responseWrapper, start, caught);
            MDC.remove(TransactionIds.MDC_KEY);
        }
    }

    private void safeLogCompletion(BoundedRequestCaptureWrapper request,
                                   BoundedResponseCaptureWrapper response,
                                   long start, Throwable caught) {
        try {
            logCompletion(request, response, start, caught);
        } catch (RuntimeException ignored) {
            try {
                INTERNAL_LOGGER.warn("http_access_log_completion_failed");
            } catch (RuntimeException warningFailure) {
                // access logging은 best effort다. 경고 실패도 API 응답이나 원래 예외를 바꾸지 않는다.
            }
        }
    }

    private void logCompletion(BoundedRequestCaptureWrapper request,
                               BoundedResponseCaptureWrapper response,
                               long start, Throwable caught) {
        ...
        String responseBody = caught != null
                ...
                : bodyMasker.maskResponse(response, response.getCapturedBytes(),
                        response.getObservedByteCount(), response.isOverflowed());
