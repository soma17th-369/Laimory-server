...
/**
 * 푸시 설정 화면의 use case orchestration — 마스터와 일일 알림 설정 두 leaf service를 합성한다.
...
public class PushSettingService {
    ...
    public PushSettingsResponse getSettings(String applicationVersion, UUID subjectId) {
        ...
        DailyNotificationPreferenceService.Settings dailyReminder =
                dailyNotificationPreferenceService.findSettings(subjectId);
    ...
    public void updateDailyReminderEnabled(String applicationVersion, UUID subjectId, Boolean enabled) {
        // applicationVersion: 버전별 처리 분기 지점(현재 단일 버전이라 분기 없음).
        dailyNotificationPreferenceService.updateEnabled(subjectId, requireEnabled(enabled));
    }
