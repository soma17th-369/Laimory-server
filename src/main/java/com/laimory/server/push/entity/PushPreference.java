...
/**
 * subject별 FCM 전체 수신 마스터 행. 이 행 하나가 정보성·광고성을 가리지 않고 모든 푸시의 최상위
 * ON/OFF다 — 종류별 설정({@link ScheduledNotificationPreference})과 법적 동의
 * ({@link NotificationConsent})는 이 마스터를 통과한 뒤에만 의미가 있다.
 *
...
public class PushPreference extends BaseEntity {
    ...
    /** 기본 ON — 기존 정보성 알림 동작을 보존한다(광고성은 별도 동의가 추가로 필요). */
