 ...
 *
 * <p>{@code nextDueAt}과 {@code notificationTime}은 {@code Asia/Seoul} 벽시계 계약이고
 * {@code lastProcessedOccurrenceDate}는 실제 claim 시각의 날짜가 아니라 <b>처리한 예정 occurrence의
 * KST 날짜</b>다 — 지연 복구가 다음 날짜 알림을 잡아먹지 않게 하는 기준이다.
 *
...
public class ScheduledNotificationPreference extends BaseEntity {
    ...
    /** 종류별 수신 여부. 광고성 종류는 ON 전환 시 법적 동의를 먼저 요구한다. */
    ...
    /** 마지막으로 처리(발송 또는 skip)한 occurrence의 KST 날짜. 신규 행은 NULL이다. */
    @Column(name = "last_processed_occurrence_date")
    private LocalDate lastProcessedOccurrenceDate;
