 ...
 *
 * <p>{@code GET /initializer}가 읽는 값을 바꾸는 일반 앱용 경로다. 완료는 단방향이라
 * 일반 앱 API에 {@code false}로 되돌리는 짝을 두지 않으며, 상태의 owner는 {@code @CurrentSubject}가 JWT principal에서
...
public interface OnboardingApi {
    ...
    @Operation(summary = "앱 온보딩 완료 기록",
            description = "인증 subject의 온보딩 완료 여부를 `true`로 기록한다. request body는 없다 — "
                    ...
                    + "일반 앱용으로 제공하지 않으며, 약관 동의 이력은 이 값을 바꾸지 않는다. 설정 행이 없는 사용자는 "
