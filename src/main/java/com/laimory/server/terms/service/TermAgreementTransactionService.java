...
/**
 * 동의 batch insert의 명시적 transaction 경계 — 검증을 끝낸 문서 복합 key 목록을 한 DB transaction으로
...
public class TermAgreementTransactionService {
    ...
    @Transactional
    public void recordAgreements(Long userId, List<TermDocumentId> documentIds, LocalDateTime acceptedAtKst) {
        // native insert는 JPA auditing을 우회한다 — 감사 시각은 batch 시작 전에 한 번 캡처해 함께 바인딩.
        LocalDateTime auditNow = LocalDateTime.now(clock);
        for (TermDocumentId documentId : documentIds) {
            termAgreementRepository.insertIfAbsent(userId, documentId.getTermType().name(),
                    documentId.getVersion(), acceptedAtKst, auditNow);
        }
    }
}
