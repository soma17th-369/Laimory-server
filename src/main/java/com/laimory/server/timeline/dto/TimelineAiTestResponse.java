...
/**
 * dev 전용 AI 동기 테스트 endpoint의 성공 응답 — 서버가 발행한 {@code taskId}, 시간 초과 여부,
 * AI 추론 결과다. app {@code ApiResponse} envelope을 쓰지 않는다.
 *
 ...
 * 응답 전체로 보면 결과 저장 계약에 {@code taskId}·{@code timedOut} 두 키를 더한 형태이므로 계약 대조는
 * {@code events} 서브트리를 기준으로 한다.
 */
public record TimelineAiTestResponse(
        ...
        String taskId,
        @Schema(description = "AI가 제한 시간이 끝나 마지막 확정본을 돌려줬으면 true. 실패가 아니라 "
                + "비동기 경로가 저장하는 값과 같은 결과라는 뜻이다")
        boolean timedOut,
