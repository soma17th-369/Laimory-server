 ...
 * {@code errorCode}·{@code error}만 로그에 남기고 DB는 건드리지 않는다.
 *
 * <p>{@code userMemory}는 <b>부분 병합이 아니라 문서 전체</b>다. 서버는 이 문서를 파싱·정규화·검증하지
 * 않고 그대로 저장한다 — 스키마를 소유하고 검증하는 쪽이 AI 서버라, 받은 것을 왕복시키면 항상 유효하다.
 */
public record AiUserMemoryUpdateResultRequest(
        ...
        Integer errorCode,
        @Schema(description = "AI 실패 설명. 로그로만 쓴다", nullable = true)
        String error
) {
