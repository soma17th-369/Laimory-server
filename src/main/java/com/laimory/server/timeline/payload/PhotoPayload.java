 ...
 * draft 저장 전 enrich 또는 Event PATCH writer가 서버 값으로만 구성한다</b>(mass assignment 방어).
 ...
 * 재구성이 채운다. <b>draft 경로에서만 채워진다</b> — Event PATCH의 수동 PHOTO 추가는 enrich를 타지 않아
 * 두 필드 없이 저장되므로, 같은 타입에 주소가 있는 사진과 없는 사진이 공존한다. 좌표가 없는 PHOTO도
 * 조회 대상이 아니라 두 필드가 비어 있다({@link StayPayload}와 같은 의미 — key 생략=주소 부재이거나 미연동
 * 이거나 허용된 실패, 빈 배열=장소 없음 또는 허용된 실패).
 *
 * <p>Event PATCH의 수동 PHOTO 입력은 이 저장 DTO를 request 타입으로 재사용하지 않는다. 클라이언트에서는
