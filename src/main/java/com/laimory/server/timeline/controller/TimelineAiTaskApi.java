 ...
 *
 * <p>dispatch가 준 단일 task token을 세 endpoint가 공통으로 검증하고, Redis task stage가 호출 순서를
 * 제한한다.
 */
@Tag(name = "Timeline AI Task (서버간)",
        description = "AI 서버 ↔ API 서버 작업 입력·결과(/s/api) — 단일 Task-Token 헤더로 인증")
...
public interface TimelineAiTaskApi {
    ...
    @Operation(summary = "AI 작업 입력 조회",
            description = "AI 추론에 필요한 정규 입력(기록 날짜·timezone·window·source item)을 반환한다. "
                    ...
                    + "Task-Token은 dispatch body로 전달된 단일 task token이며, 입력 응답 성공 뒤 결과 대기 "
                    + "단계로 전이하고 작업 TTL을 다시 확보한다. 응답 유실 시 같은 토큰으로 재조회할 수 있다.")
    @ApiResponses({
    ...
    @Operation(summary = "AI 생성 결과 저장",
            description = "AI가 만든 Event와 채택한 source rawId를 받아 Event/Item/junction 저장과 채택 source "
                    ...
                    + "Redis 결과 저장 단계를 선점한 요청만 graph를 쓰며, 저장 완료 뒤 재시도는 no-op 성공이다. "
                    + "작업 상태 전이(SUCCESS/FAILED)는 이 API가 아니라 콜백 API가 기록한다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200",
                    description = "저장 완료 또는 이미 반영된 재시도(본문 없음)"),
    ...
    ResponseEntity<Void> result(@Parameter(description = "API 버전", example = "v1")
                                @PathVariable String applicationVersion,
                                @Parameter(description = "draft 작업 ID")
                                @PathVariable String taskId,
                                @Parameter(description = "task 인증 토큰")
