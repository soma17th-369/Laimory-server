...
public interface TimelineAiTaskApi {
    ...
    @Operation(summary = "AI 작업 입력 조회",
            description = "AI 추론에 필요한 정규 입력(기록 날짜·timezone·window·source item)을 반환한다. "
                    ...
                    + "응답 유실 뒤 이미 소비한 token으로 재시도하면 같은 입력을 다시 조립해 새 result "
                    + "token과 함께 반환한다(첫 요청 도착 기준 재시도 창 안에서만). 마지막으로 받은 응답의 "
                    + "taskToken만 유효하다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200",
                    description = "입력 반환. 재시도면 같은 입력 + 새 result token"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401",
                    description = "`-1002` — 토큰 누락·불일치, 재시도 창 만료"),
    ...
    @Operation(summary = "AI 생성 결과 저장",
            description = "AI가 만든 Event와 채택한 source rawId를 받아 Event/Item/junction 저장과 채택 source "
                    ...
                    + "결과 저장을 선점한 요청만 graph를 쓰며, 성공 응답은 callback용 새 taskToken을 body로 "
                    + "반환한다. 응답 유실 뒤 이미 소비한 token으로 **같은 결과**를 재시도하면 graph를 다시 "
                    + "쓰지 않고 `status=ALREADY_PROCESSED`와 새 callback token만 반환한다(첫 요청 도착 기준 "
                    + "재시도 창 안에서만, payload 지문이 일치할 때만). 마지막으로 받은 응답의 taskToken만 "
                    + "유효하다. 작업 상태 전이(SUCCESS/FAILED)는 이 API가 아니라 콜백 API가 기록한다 — "
                    + "ALREADY_PROCESSED도 성공이므로 AI는 SUCCESS 콜백을 보내야 종결된다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200",
                    description = "저장 완료(`STORED`) 또는 동일 결과 재시도(`ALREADY_PROCESSED`) "
                            + "+ callback용 `taskToken`(재시도마다 새로 발급)"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400",
                    description = "`-400` — 결과 계약 위반(빈 결과, 필수 필드 누락, 이 작업의 source가 아닌 rawId 등)"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401",
                    description = "`-1002` — 토큰 누락·불일치, 재시도 창 만료, commit 전 장애로 저장 증거 없음"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404",
                    description = "`-1001` — 작업 없음(만료 포함), `-404` — 작업의 하루 기록 없음"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409",
                    description = "`-1017` — 이미 종결된 작업, 최초 저장분과 다른 결과의 재시도, "
                            + "처리 중인 결과 저장의 중복 요청, `-1003` — 이미 저장(SAVED)된 하루 기록")
    })
