...
public interface TimelineAiTaskApi {
    ...
    @Operation(summary = "AI 생성 결과 저장",
            description = "AI가 만든 Event와 채택한 source rawId를 받아 Event/Item/junction 저장과 채택 source "
                    ...
                    + "반환한다. 응답 유실 뒤 이미 소비한 token으로 재시도하면 graph를 다시 쓰지 않고 새 "
                    + "callback token만 반환한다(첫 요청 도착 기준 재시도 창 안에서만). 이때 body는 적용할 "
                    + "경로가 없어 무시한다. 신규 저장과 재시도의 응답 shape는 같으며, 마지막으로 받은 응답의 "
                    + "taskToken만 유효하다. 작업 상태 전이(SUCCESS/FAILED)는 이 API가 아니라 콜백 API가 "
                    + "기록한다 — 재시도 응답도 성공이므로 AI는 SUCCESS 콜백을 보내야 종결된다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200",
                    description = "저장 완료 또는 재시도 + callback용 `taskToken`(재시도마다 새로 발급)"),
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404",
                    description = "`-1001` — 작업 없음(만료 포함), `-404` — 작업의 하루 기록 없음"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409",
                    description = "`-1017` — 이미 종결된 작업, 처리 중인 결과 저장의 중복 요청, "
                            + "`-1003` — 이미 저장(SAVED)된 하루 기록")
    })
