 ...
 * PHOTO S3 key 수집(guard 안에서 — 동시 finalize가 수집과 삭제 사이에 item을 추가하지 못함) →
 * S3 배치 삭제(DB 트랜잭션 밖) → 전 batch 성공 시에만 별도 빈 트랜잭션에서 재확인 후 DB 삭제
 * ({@link TimelineDeletionTransactionService} — 하위 행은 DB {@code ON DELETE CASCADE}).
 *
...
public class TimelineDeletionService {
    ...
    /** Event와 하위 Items를 삭제한다(사진 S3 객체 포함). 마지막 Event를 지워도 DailyRecord는 유지된다. */
    public void deleteEvent(String applicationVersion, long userId, Long timelineEventId) {
        ...
        deleteUnderDateGuard(userId, record.getRecordDate(), "event", timelineEventId,
                () -> timelineItemService.findByTimelineEventId(timelineEventId),
                () -> timelineDeletionTransactionService.deleteEvent(userId, timelineEventId));
    }

    /** 하루 전체(Record·Events·Items)를 삭제한다(사진 S3 객체 포함). */
    public void deleteDailyRecord(String applicationVersion, long userId, Long dailyRecordId) {
        ...
        deleteUnderDateGuard(userId, record.getRecordDate(), "dailyRecord", dailyRecordId,
                () -> findItemsOfAllEvents(dailyRecordId),
                () -> timelineDeletionTransactionService.deleteDailyRecord(userId, dailyRecordId));
    }

    /**
     * 날짜 guard 선점 → PHOTO key 수집 → S3 배치 삭제 → DB 삭제(별도 빈 트랜잭션) → finally 해제.
     * itemsSupplier는 guard 선점 <b>후</b>에 평가한다 — 동시 finalize(같은 guard 필요)가 수집과 삭제
     * 사이에 PHOTO item을 추가해 S3 orphan을 만드는 창을 없앤다.
     */
    ...
    /** 하루 기록의 모든 이벤트 하위 item을 모은다(leaf 서비스 합성 — 레포 직접 접근 금지). */
    private List<TimelineItem> findItemsOfAllEvents(Long dailyRecordId) {
        List<TimelineItem> items = new ArrayList<>();
        for (TimelineEvent event : timelineEventService.findByDailyRecordId(dailyRecordId)) {
            items.addAll(timelineItemService.findByTimelineEventId(event.getTimelineEventId()));
        }
        return items;
    }

    /**
     * PHOTO item들의 payload에서 filename을 복원해 full S3 key를 유도한다(중복 제거). payload가 깨졌거나
     * filename이 없으면 S3 삭제는 건너뛰고 계속 진행한다(orphan 허용 —
     * {@code TimelineDraftCleanupScheduler.deletePhotoObject}와 동일 규칙; 나쁜 한 건이 전체 삭제를 막지 않음).
     * userId는 소유권 검증을 통과한 컨트롤러 결정값이라 {@code DailyRecord.userId}와 동일하다.
     */
