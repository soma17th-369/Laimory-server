 ...
 *
 * <p>불변식: 보관기간(retentionDays, 기본 7일)은 PROCESSING_TTL(2분)보다 훨씬 커야 한다(retention ≫ PROCESSING_TTL 2m).
...
public class TimelineDraftCleanupScheduler {
    ...
    /**
     * 허용 최소 보관기간(일). 1일(24h)이면 PROCESSING_TTL(2m)보다 충분히 크다.
    ...
    /**
     * 클래스 불변식(retention ≫ PROCESSING_TTL 2m)을 기동 시점에 fail-fast로 강제한다.
    ...
    void validateRetentionDays() {
        if (retentionDays < MIN_RETENTION_DAYS) {
            throw new IllegalStateException(
                    "app.draft.retention-days는 최소 %d 이상이어야 한다(현재 %d). retention은 PROCESSING_TTL(2m)보다 훨씬 커야 active draft가 조기 삭제되지 않는다."
