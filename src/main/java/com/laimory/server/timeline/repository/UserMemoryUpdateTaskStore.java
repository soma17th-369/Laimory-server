 ...
 * guard가 {@code timeline:user-memory-update:user:{base64url(subjectId)}}(SET NX, TTL).
...
public class UserMemoryUpdateTaskStore {
     ...
     */
    public boolean acquireGuard(SubjectId subjectId, String taskId, Duration ttl) {
     ...
     */
    public void releaseGuard(SubjectId subjectId) {
        redis.delete(guardKey(subjectId));
    }

    static String guardKey(SubjectId subjectId) {
        return GUARD_KEY_PREFIX + SubjectIdCodec.encode(subjectId);
    }
}
