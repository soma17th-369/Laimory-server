...
-- 테이블을 새로 만들지 않으려고 이 한 행에 모은다. 지금 담긴 것은 전체 push 마스터 하나뿐이다.
-- push_enabled는 모든 push의 전체 ON/OFF이며 기본은 ON이다 — 예정 알림과 타임라인 완료 통지가
-- 모두 이 값을 읽는다(#367: 마스터 OFF가 곧 전체 차단이라야 탈퇴가 FID를 지우지 않고 발송만 막는다).
