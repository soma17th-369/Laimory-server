...
/**
 * dev 전용 AI 동기 테스트 endpoint 배선 검증({@link ApplicationContextRunner}) — 노출 스위치를 실제 컨텍스트
 * 기동으로 확인한다({@code AiDispatcherWiringTest} 선례).
 *
 ...
 * (경로 자체가 없으므로 404), true면 각각 정확히 하나여야 한다. 설정 값 자체의 fail-fast 검증은
 * {@link TimelineAiTestClient} 생성자가 소유하며 {@link TimelineAiTestClientTest}가 고정한다.
 */
class TimelineAiTestWiringTest {
    ...
    private final ApplicationContextRunner runner = new ApplicationContextRunner()
            ...
            .withUserConfiguration(TimelineAiTestClient.class, TimelineAiTestService.class,
                    TimelineAiTestController.class);
    ...
    @Test
    void failsFastWhenEnabledWithoutUsableAiUrl() {
        // 켜 두고 URL을 안 주면 첫 호출이 아니라 기동에서 실패해야 한다.
        runner.withPropertyValues("app.ai.timeline-test.enabled=true")
