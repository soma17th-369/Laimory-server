...
/**
 * {@code app.ai.mode} 배선 검증({@link ApplicationContextRunner}) — noop/fake dispatcher 선택과
 ...
 *
 * <p>fake dispatcher 생성자의 {@code RestClient.Builder}는 수동 빈이 아니라 production과 같은 Boot
 * 자동설정으로 제공한다 — 자동설정이 깨져 dev fake context가 기동 실패하는 회귀를 수동 빈이 가리는 것을
 * 막는다. staging은 실제 빈으로 등록해 {@code app.ai.mode=fake} 조건을 함께 검증하고, leaf dependency만
 * mock한다. dispatcher는 호출하지 않으므로 HTTP·sleep·DB·Redis가 필요 없다.
 */
class AiDispatcherWiringTest {
    ...
    private final ApplicationContextRunner runner = new ApplicationContextRunner()
            ...
            // fake dispatcher 생성자의 @Value "2s" → Duration 변환은 production에선 SpringApplication이
            ...
            .withBean(TimelineDraftEventSuggestionService.class,
                    () -> Mockito.mock(TimelineDraftEventSuggestionService.class))
            .withUserConfiguration(
                    AsyncConfig.class,
                    NoOpTimelineEventSuggestionDispatcher.class,
                    FakeAiTimelineEventSuggestionDispatcher.class,
                    FakeAiEventSuggestionStagingService.class,
    ...
    static class DispatcherConsumer {
        DispatcherConsumer(TimelineEventSuggestionDispatcher dispatcher) {
        }
    }
    ...
    void modeMissing_defaultsToNoOpDispatcher() {
        ...
        runner.run(context -> {
            assertThat(context).hasNotFailed().hasSingleBean(TimelineEventSuggestionDispatcher.class);
            assertThat(context.getBean(TimelineEventSuggestionDispatcher.class))
                    .isInstanceOf(NoOpTimelineEventSuggestionDispatcher.class);
            assertThat(context).doesNotHaveBean(FakeAiEventSuggestionStagingService.class);
        });
    }
    ...
    @Test
    void fakeMode_wiresFakeDispatcherWithAsyncProxyAndStaging() {
        ...
        runner.withPropertyValues(
                ...
                .run(context -> {
                    assertThat(context).hasNotFailed()
                            .hasSingleBean(TimelineEventSuggestionDispatcher.class)
                            .hasSingleBean(FakeAiEventSuggestionStagingService.class);
                    TimelineEventSuggestionDispatcher dispatcher =
                            context.getBean(TimelineEventSuggestionDispatcher.class);
                    ...
                    assertThat(AopProxyUtils.ultimateTargetClass(dispatcher))
                            .isEqualTo(FakeAiTimelineEventSuggestionDispatcher.class);
                });
    }

    @Test
    void unknownMode_failsContext() {
        // fail-fast 경로 고정: 오타 → noop/fake 어느 조건도 매칭 안 됨 → dispatcher 빈 부재로 consumer
        // 주입 실패가 root cause여야 한다(무관한 오류가 아니라 이 경로임을 못박음).
        runner.withPropertyValues("app.ai.mode=fak")
                .run(context -> assertThat(context).getFailure()
                        .rootCause()
                        .isInstanceOf(NoSuchBeanDefinitionException.class)
                        .hasMessageContaining("TimelineEventSuggestionDispatcher"));
    }
}
