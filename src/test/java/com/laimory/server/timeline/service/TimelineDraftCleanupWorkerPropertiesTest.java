...
class TimelineDraftCleanupWorkerPropertiesTest {
    ...
    void assignsEverySlotExactlyOnceForTwoServers() {
        for (int count : new int[] {1, 2}) {
            ...
            for (int server = 0; server < 2; server++) {
                var properties = new TimelineDraftCleanupWorkerProperties(true, 7, 250, server, 2, count);
    ...
    void rejectsInvalidWorkerTopology() {
        assertThatIllegalStateException().isThrownBy(() -> new TimelineDraftCleanupWorkerProperties(true, 7, 250, -1, 2, 1));
        assertThatIllegalStateException().isThrownBy(() -> new TimelineDraftCleanupWorkerProperties(true, 7, 250, 2, 2, 1));
        assertThatIllegalStateException().isThrownBy(() -> new TimelineDraftCleanupWorkerProperties(true, 7, 250, 0, 0, 1));
        assertThatIllegalStateException().isThrownBy(() -> new TimelineDraftCleanupWorkerProperties(true, 7, 250, 0, 2, 0));
        assertThatIllegalStateException().isThrownBy(() -> new TimelineDraftCleanupWorkerProperties(true, 7, 1001, 0, 2, 1));
    }
}
