...
/**
 * Redis 접근은 환경 prefix(dev_ 등)가 강제되도록 PrefixedRedis facade만 거쳐야 한다.
...
class RedisAccessArchTest {
    ...
    static final ArchRule redis_access_only_through_facade =
            noClasses()
                    .that().doNotHaveFullyQualifiedName("com.laimory.server.common.redis.PrefixedRedis")
                    .should().dependOnClassesThat().resideInAPackage("org.springframework.data.redis..")
                    .because("Redis 접근은 환경 prefix가 강제되도록 PrefixedRedis facade만 거쳐야 한다");
}
